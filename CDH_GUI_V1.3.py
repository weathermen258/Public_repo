...
def select_var_window():
    ...
    variables_manual_2 = ['Khí áp','Mực nước','Lưu Lượng', 'Tmax','Tmin']
...
def get_manual_data():
    ...
    if (method_choice == 'Quan trắc thủ công'):
        output_file = os.path.join(cwd+"/Output/",region_short+'_Data_manual_'+str(start_date)+"_"+str(end_date)+".xlsx")
    elif (method_choice == 'Mưa tự động'):
        output_file = os.path.join(cwd+"/Output/",region_short+'_Data_auto_'+str(start_date)+"_"+str(end_date)+".xlsx")
    elif (method_choice == 'Số liệu hồ chứa'):
        output_file = os.path.join(cwd+"/Output/",region_short+'_Data_reservor_'+str(start_date)+"_"+str(end_date)+".xlsx")
    elif (method_choice == 'Quan trắc biển'):
        output_file = os.path.join(cwd+"/Output/",region_short+'_Data_sea_'+str(start_date)+"_"+str(end_date)+".xlsx")
    else:
        show_error()
    ...
    for name in vars_name:
        ...
        ###################
         ...
         var
           ...
           str(start_date)+"%20"+begin_hour+":00:00&to="+str(end_date)+"%20"+finish_hour+":00:00"
            print (url_2)
            data_file = os.path.join(cwd+ "/Data/",name+"Sea_data_"+str(start_date)+"_"+str(end_date)+".xlsx")
            urllib.request.urlretrieve(url_2,data_file)
        elif (method_choice == 'Số liệu hồ chứa'):
            url= "https://cdh.vnmha.gov.vn/KiWIS/KiWIS?service=kisters&type=queryServices&request="+\
            ...
            print (url_2)
            data_file = os.path.join(cwd+ "/Data/",region_choice+name+"Res_data_"+str(start_date)+"_"+str(end_date)+".xlsx")
            urllib.request.urlretrieve(url_2,data_file)
