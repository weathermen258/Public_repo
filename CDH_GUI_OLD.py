...
def get_manual_data():
    ...
    for var in var_s:
        if (method_choice == 'Quan trắc thủ công'):
         ...
         var+"/Manual.O"
            print (url)
            stations_file =os.path.join(cwd+'/Data/',var+"_station_"+str(start_date)+"_"+str(end_date)+".xlsx")
            print (stations_file)
           ...
           str(start_date)+"%20"+begin_hour+":00:00&to="+str(end_date)+"%20"+finish_hour+":00:00"
            print (url_2)
            data_file = os.path.join(cwd+ "/Data/",var+"Manual_data_"+str(start_date)+"_"+str(end_date)+".xlsx")
        ...
        ###################
            ...
            url_2= "https://cdh.vnmha.gov.vn/KiWIS/KiWIS?service=kisters&type=queryServices"+\
                   "&request=getTimeseriesValues&datasource=0&format=xlsx&ts_path=5/*/Precip/6h.total"\
                   +"&returnfields=Timestamp,Value&metadata=true&"+\
                   "from="+str(start_date)+"%20"+begin_hour+":00:00&to="+str(end_date)+"%20"+finish_hour+":00:00"
            print (url_2)
            data_file = os.path.join(cwd+ "/Data/",var+"Auto_data_"+str(start_date)+"_"+str(end_date)+".xlsx")
            urllib.request.urlretrieve(url_2,data_file)
        elif (method_choice == 'Số liệu hồ chứa'):
            url= "https://cdh.vnmha.gov.vn/KiWIS/KiWIS?service=kisters&type=queryServices&request="+\
            ...
            print (url_2)
            data_file = os.path.join(cwd+ "/Data/",var+"Res_data_"+str(start_date)+"_"+str(end_date)+".xlsx")
            urllib.request.urlretrieve(url_2,data_file)
    ...
    Label(root,text ="\n Đã lấy xong dữ liệu!").place(x=170,y=250)
