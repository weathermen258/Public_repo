...
def get_record_synop(str0):
   ...
   for i in range(typh_len):
      if (str0.split()[i][0:1] == '5') and\
         (pos('333',str0) <= pos(str0.split()[i],str0) <= pos('333',str0) + 5)\
         and (str0.split()[i] != '555'):
         deltap_exist += 1
         #print(str0.split()[i])
...
## function to get data from typh KT
...
###################################
def send_typh(file_name):
   from ftplib import FTP
   ftp = FTP('10.152.40.227')
   ftp.login()
   ftp.cwd('typh')
...
def write_to_excel():
   import os.path
   output_excel_file = os.path.join(cwd,'TYPH_' + str(ty_name) + '.xlsx')
   ...
   sheet0 = wb['DATA']
   sheet0['I1'] = 'TYPH ' + str(ty_name).upper()
   i = 0
   ###
   global typh_stations,typh_hour0,typh_min0,wind_dir0,wind_speed0,slp0,\
      delta_p0,gust_dir0,gust_speed0,pmin0,time_pmin0,dir_max0,wind_max0,\
      time_wind_max0
...
yYear='{}' and mMonth='{}' and dDay='{}' and hHour='{}'".format(ty_name,\
   ...
   #send_typh('TYPH_' + str(ty_name) + '.xlsx')
   messagebox.showinfo("Ket qua","Du Lieu Da Duoc Ghi ra file Excel")
   return
##################################
def get_input():
   global input_file, ty_name, obs_year, obs_month
   ty_name = ty_name_entry.get()
   obs_year = obs_year_entry.get()
   obs_month = obs_month_entry.get()
   input_file = ''
   input_file = askopenfilename()
   #messagebox.showinfo("input complete\n", "typhoon name: "+ty_name+"\n"+\
                       #"Year: "+obs_year+"\n"+ "Month: "+obs_month+"\n"+\
...
ttk.Button(mainframe, text="Giai Ma", command=write_data_to_db)\
                      .grid(column=2, row=2, sticky=W)

ttk.Button(mainframe, text="Ghi Du Lieu", command=write_to_excel)\
