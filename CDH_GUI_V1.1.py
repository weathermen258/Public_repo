...
ttk.Label(root, text="Chọn khu vực:").\
                 place(x=10,y =30)
regions = ['Đồng Bằng Bắc Bộ','Bắc Trung Bộ',\
           'Trung Trung Bộ']
region = StringVar()
region.set('Bắc Trung Bộ')
drop_region = OptionMenu(root,region,*regions)
drop_region.place(x=110,y =25)
###########################################
...
def select_var_window():
    variables_manual = ['Nhiệt độ','Lượng mưa','Độ ẩm','Hướng gió',\
             'Tốc độ gió','Khí áp','Mực nước','Lưu Lượng', 'Tmax','Tmin']
...
def get_manual_data():
    ...
    if (method_choice == 'Quan trắc thủ công'):
        output_file = os.path.join(cwd+"/Output/",region_choice+'Data_manual_'+str(start_date)+"_"+str(end_date)+".xlsx")
    elif (method_choice == 'Mưa tự động'):
        output_file = os.path.join(cwd+"/Output/",region_choice+'Data_auto_'+str(start_date)+"_"+str(end_date)+".xlsx")
    elif (method_choice == 'Số liệu hồ chứa'):
        output_file = os.path.join(cwd+"/Output/",region_choice+'Data_reservor_'+str(start_date)+"_"+str(end_date)+".xlsx")
    else:
        show_error()
